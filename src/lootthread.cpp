...
namespace lootcli
{
				...
				try {
					...
					if (newSettings.Type() == m_GameSettings.Type()) {
						...
						if (source) {
							...
							auto migratedSource = migrateMasterlistRepoSettings(newSettings.Id(), *url, *branch);
	...
	bool LOOTWorker::IsEnderal(const toml::table& table) { return IsEnderal(table, "enderal"); }
	...
	std::string LOOTWorker::getOldDefaultRepoUrl(loot::GameId GameId) {
		...
		default:
			throw std::runtime_error(
				...
				std::to_string(
					static_cast<std::underlying_type_t<loot::GameId>>(GameId)));
		}
	}
	...
	std::optional<std::string> LOOTWorker::migrateMasterlistRepoSettings(loot::GameId GameId,
		std::string url,
		std::string branch) {
	...
	int LOOTWorker::run()
	{
		...
		try {
			...
			std::unique_ptr<loot::GameInterface> gameHandle = CreateGameHandle(
				m_GameSettings.Type(), m_GamePath, profile.string());
		...
		const std::vector<loot::PluginCleaningData>& data) const
	{
		...
		for (const auto& d : data) {
			...
			auto simpleMessage = loot::SelectMessageContent(loot::Message(loot::MessageType::say, d.GetDetail()).GetContent(), m_Language);
			if (simpleMessage.has_value()) {
